import 'dart:async';
import 'dart:io';
import 'package:bloc_login/new/shared.pref.dart';
...
class AuthBloc extends HydratedBloc<AuthEvent, AuthState> {
  AuthBloc({AuthenticationRepository authenticationRepository})
    ...
    on<AuthRegisterRequested>(_onAuthenticationRegisterRequested);
    on<AuthForgetPasswordRequested>(_onAuthenticationForgetPasswordRequested);
    on<AuthRefreshRequested>(_onAuthenticationLoginRefresh);
    on<AuthResetpasswordRequested>(_onAuthenticationResetpasswordRequested);
  ...
  Future<FutureOr<void>> _onAuthenticationLoginRequested(AuthLoginRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      emit(AuthLoading());
      LogUser loguser = await _sharedpreferencesData.logIn(username: event.email, password: event.password, device: event.device);
      if (loguser != null) {
        emit(AuthGranted(loguser.token, loguser.user));
      } else {
        emit(const AuthDenied(["Login failed"]));
      }
    } catch (e) {
  ...
  Future<FutureOr<void>> _onAuthenticationRegisterRequested(AuthRegisterRequested event, Emitter<AuthState> emit) async {
    try {
      final SharedpreferencesData _sharedpreferencesData = SharedpreferencesData();
      emit(AuthLoading());
      LogUser loguser = await _sharedpreferencesData.register(username: event.email, password: event.password, device: event.device);
      if (loguser != null) {
        emit(AuthGranted(loguser.token, loguser.user));
      } else {
        emit(const AuthDenied(["Cannot create this user"]));
      }
    } catch (e) {
      if (kDebugMode) {
        print(e);
  ...
  Future<FutureOr<void>> _onAuthenticationResetpasswordRequested(AuthResetpasswordRequested event, Emitter<AuthState> emit) async {
    try {
      final SharedpreferencesData _sharedpreferencesData = SharedpreferencesData();
      emit(AuthLoading());
      LogUser loguser = await _sharedpreferencesData.reset(username: event.email, password: event.password);
      if (loguser != null) {
        emit(AuthGranted(loguser.token, loguser.user));
      } else {
        emit(const AuthDenied(["Cannot Find this user"]));
      }
    } catch (e) {
      if (kDebugMode) {
        print(e);
  ...
  Future<FutureOr<void>> _onAuthenticationForgetPasswordRequested(AuthForgetPasswordRequested event, Emitter<AuthState> emit) async {
    try {
      final SharedpreferencesData _sharedpreferencesData = SharedpreferencesData();
      emit(AuthLoading());
      var loguser = await _sharedpreferencesData.forgetpassword(
        username: event.email,
      );
      if (loguser != null) {
        // assume sending code by email form here 1234
        emit(Authsendcode(event.email));
      } else {
        emit(const AuthDenied(["Cannot Find this user"]));
      }
    } catch (e) {
